...
import { Search, CheckCircle, XCircle, Clock, BookOpen } from 'lucide-react'
import ToastContainer from '../../components/Toast'
import { useApi, useToast } from '../../hooks/useApi'
import { getReservations, cancelReservation, confirmReservation } from '../../services/api'
...
export default function ModReservations() {
  ...
  const handleConfirm = async (id) => {
    try { await confirmReservation(id); refetch(); showToast('تم تأكيد الحجز ✅') }
  ...
  return (
    <div style={{ padding: isMobile ? '16px 14px' : '28px 24px', maxWidth: 1200, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 12, flexDirection: isMobile ? 'column' : 'row', alignItems: isMobile ? 'flex-start' : 'center' }}>
        <div>
          <h2 style={{ fontSize: 20, fontWeight: 800 }}>مراجعة الحجوزات</h2>
          <p style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 3 }}>{pendingCount} حجز معلق يحتاج مراجعة</p>
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          <div style={{ padding: '8px 14px', background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.25)', borderRadius: 8 }}>
            <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>معلقة: </span>
            <span style={{ fontWeight: 700, color: 'var(--amber)' }}>{pendingCount}</span>
          </div>
          <div style={{ padding: '8px 14px', background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.25)', borderRadius: 8 }}>
            <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>مؤكدة: </span>
            <span style={{ fontWeight: 700, color: '#34d399' }}>{confirmedCount}</span>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
        ...
        {['all','pending','confirmed','cancelled'].map(s => (
          <button key={s} onClick={() => setStatusFilter(s)} className="btn" style={{ background: statusFilter===s?'var(--calm)':'var(--surface)', color: statusFilter===s?'white':'var(--text-secondary)', border: `1px solid ${statusFilter===s?'var(--calm)':'var(--border)'}`, fontFamily: 'var(--font)' }}>
            {{ all:'الكل', pending:'معلقة', confirmed:'مؤكدة', cancelled:'ملغية' }[s]}
          </button>
        ))}
      </div>
              ...
              {filtered.map(r => {
                ...
                return (
                  <tr key={r.id}>
                    ...
                    <td>
                      <span className={`badge ${statusConfig[r.status]?.class}`}>
                        {StatusIcon && <StatusIcon size={14}/>} {statusConfig[r.status]?.label}
                      </span>
                    </td>
                    <td>
